"""
URL リストから HTML を取得し、テキスト化して知識 DB（JSONL）を生成する。
外部ライブラリは使わず、標準ライブラリで簡易抽出を行う。
"""
...
def build_db(urls: List[str], output_path: str) -> None:
...
def main() -> None:
    ...
    output_path = os.path.join(base_dir, "db", "knowledge_chunks.jsonl")

    urls = read_urls(urls_path)
    if not urls:
        raise ValueError("urls.txt に URL がありません。")

    build_db(urls, output_path)

    meta = {
        "generatedAt": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "sourceCount": len(urls),
        "chunkCount": sum(1 for _ in open(output_path, "r", encoding="utf-8")),
        "note": "Eigen update knowledge DB",
    }
    with open(os.path.join(base_dir, "db", "index.json"), "w", encoding="utf-8") as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)

    print(f"✅ knowledge DB を作成しました: {output_path}")
